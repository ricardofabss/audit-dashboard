...
import { Bell, Bot, Command, Moon, Search, Sun } from "lucide-react";
...
export function Topbar({ onToggleAI }: { onToggleAI: () => void }) {
  ...
  const workspaceOptions = useMemo(() => ["Global HQ", "APAC Cluster", "Jakarta Ops", "EMEA Nordics"], []);
      ...
      <div className="flex h-full items-center justify-between gap-3 px-4 md:px-6">
        <div className="flex items-center gap-3">
          <select
            ...
            className="h-9 rounded-lg border border-white/10 bg-white/[0.04] px-3 text-sm text-slate-100 outline-none focus:border-cyan-300/50"
          >
            {workspaceOptions.map((item) => (
              <option key={item} value={item} className="bg-[#0b1739]">
                {item}
              </option>
            ))}
          </select>
